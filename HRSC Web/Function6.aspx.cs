...
public partial class Function6 : System.Web.UI.Page
{
    ...
    protected void Information()
    {
        try
        {
            ...
            else
            {
                ...
                con.Open();
                SqlDataAdapter cmd = new SqlDataAdapter("SELECT cu.Name, p.Description, o.Corder_No, CONVERT(varchar, c.Date_Placed, 103) AS 'Date Placed'  FROM Product p, Oline o, Corder c, Customer cu WHERE p.Product_No = o.Product_No AND o.Corder_No = c.Corder_NO AND c.Customer_No = cu.Custno AND p.Description ='" + TextBox1.Text + "' AND c.Date_Placed = (SELECT MAX(c.Date_Placed) FROM Product p, Oline o, Corder c, Customer cu WHERE p.Product_No = o.Product_No AND o.Corder_No = c.Corder_NO AND c.Customer_No = cu.Custno AND p.Description ='" + TextBox1.Text + "')", con);
                cmd.Fill(results);
